...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; className: string; icon: React.ReactNode }> = {
    draft: { label: "Draft", className: "bg-gray-100 text-gray-700 border-gray-200", icon: <FileText className="w-3 h-3" /> },
    sent: { label: "Awaiting Payment", className: "bg-amber-50 text-amber-700 border-amber-200", icon: <Clock className="w-3 h-3" /> },
    paid: { label: "Paid", className: "bg-emerald-50 text-emerald-700 border-emerald-200", icon: <CheckCircle2 className="w-3 h-3" /> },
    overdue: { label: "Overdue", className: "bg-red-50 text-red-700 border-red-200", icon: <AlertCircle className="w-3 h-3" /> },
  };
  ...
  if (type === "annual")
    return (
      <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200 gap-1.5 px-3 py-1 text-xs font-medium">
        <CalendarDays className="w-3 h-3" />Annual Recurring
      </Badge>
    );
  return null;
}
        ...
        <Card className="max-w-md w-full">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              This invoice link is invalid or has expired. Please contact Gro Digital for assistance.
            </p>
          </CardContent>
        </Card>
      </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-muted/50">
                    <th className="text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground px-5 py-3.5">Description</th>
                    <th className="text-center text-xs font-semibold uppercase tracking-wider text-muted-foreground px-4 py-3.5 hidden sm:table-cell">Frequency</th>
                    <th className="text-center text-xs font-semibold uppercase tracking-wider text-muted-foreground px-4 py-3.5 hidden sm:table-cell">VAT</th>
                    <th className="text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground px-5 py-3.5">Amount</th>
                  </tr>
                </thead>
              ...
              <div className="max-w-xs ml-auto px-5 py-4 space-y-2">
                {!isRecurring && (
                  <>
                    ...
                    {parseFloat(String(invoice.totalAmount)) !== parseFloat(String(invoice.amountDue)) && (
                      <>
                        <div className="flex justify-between text-sm">
