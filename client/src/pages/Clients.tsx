      ...
      {isLoading ? (
        ...
        view === "card" ? (
          ...
          <div className="rounded-lg border border-border overflow-hidden shadow-sm">
            {/* Header */}
            <div className="grid grid-cols-[2fr_76px_76px_76px_76px_96px_48px] bg-muted/50 border-b border-border px-4 py-2.5 gap-2">
            ...
            <div className="divide-y divide-border bg-background">
              {clients.map((client) => {
                ...
                return (
                  <Link key={client.clientSlug} href={`/client/${client.clientSlug}`}>
                    <div className="grid grid-cols-[2fr_76px_76px_76px_76px_96px_48px] items-center px-4 py-3 hover:bg-muted/40 transition-colors cursor-pointer group gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full shrink-0 ${hasHosting ? "bg-emerald-500" : "bg-muted-foreground/20"}`} />
                        <span className={`text-xs font-medium ${hasHosting ? "text-emerald-700" : "text-muted-foreground/40"}`}>
                          {hasHosting ? "Active" : "–"}
                        </span>
                      </div>
                      ...
                      {/* Marketing */}
                      <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full shrink-0 ${hasMarketing ? "bg-violet-500" : "bg-muted-foreground/20"}`} />
                        <span className={`text-xs font-medium ${hasMarketing ? "text-violet-700" : "text-muted-foreground/40"}`}>
                          {hasMarketing ? "Active" : "–"}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          title={hasAnalytics ? "View analytics" : "Set up analytics"}
                        >
                          <span className={`w-2 h-2 rounded-full shrink-0 ${hasAnalytics ? "bg-indigo-500" : "bg-muted-foreground/20"}`} />
                          <span className={`text-xs font-medium ${hasAnalytics ? "text-indigo-700 group-hover/btn:underline" : "text-muted-foreground/40"}`}>
                            {hasAnalytics ? "Active" : "–"}
                          </span>
                        </button>
                      </div>
