...
export async function generateAndStorePostImage(prompt: string, postId: number, model?: ImageModel, style?: string, aspectRatio?: AspectRatio): Promise<string> {
  const fullPrompt = style ? `${style}. ${prompt}` : prompt;
  const { url } = await generateImage({ prompt: fullPrompt, model, aspectRatio });
